...
const SellerBlogLayout = () => {
  const { user } = useAuth();
  const location = useLocation();
  const navigate = useNavigate();

  React.useEffect(() => {
    // If we're exactly on /seller/blog, send to feed by default for sellers
    if (location.pathname === "/seller/blog") {
      navigate("/seller/blog/feed", { replace: true });
    }
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Connect with fellow sellers, share insights, and discover growth strategies from our seller community
          </p>
        </div>
        ...
        <div className="flex justify-center mb-8">
          <button
            onClick={() => navigate("/seller/blog/new")}
        ...
        <div className="flex justify-center mb-8">
          <div className="flex gap-3 p-2 bg-white/60 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20">
            <TabLink to="/seller/blog/feed" icon={Users}>Feed</TabLink>
            <TabLink to="/seller/blog/explore" icon={Compass}>Explore</TabLink>
            <TabLink to="/seller/blog/new" icon={PenTool}>Write</TabLink>
          </div>
        </div>
...
export default SellerBlogLayout;
